...
public abstract class EasyUiAssist {
	 ...
	 */
	public static String getJsonFromList(List<Object> fromList, String nameFiled, String codeFiled, String idName)
			throws ProjectException {
		if (CollectionUtils.isEmpty(fromList) || StringUtil.isNull(nameFiled) || StringUtil.isNull(codeFiled)) {
			throw new ProjectException(ExceptAll.param_error, "要解析的参数错误");
		}
		if (StringUtil.isNull(idName)) {
			idName = codeFiled;// 如果缺少ID则把code字段做为ＩＤ
		}
		String jsonTempStr = "@['{label:\"'+" + nameFiled + "+'\",value:\"'+" + codeFiled + "+'\",id:\"'+" + idName
				+ "+'\"},']";
