...
/**
 * 动态Bean现在支持的类型： eg: &lt; COL name="orderItem" alias="orderItem"
 * type="dynaBean" &gt; <br>
 * type值必须为ColType。name 如果是其它数据类型可以传string，暂不支持二进制类型
 */
public enum ColType {
	dynaBean("动态Bean", true, null), javaBean("javaBean", false, new String[] { "className" }), bytes("二进制类型", false,
			null), integer("整型", false, null), string("字符型", false, null), datetime("时间型", false, null), enums("枚举类型",
					false, new String[] { "className" });
